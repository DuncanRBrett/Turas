...
**The Stan path, executed the same evening.** Duncan installed cmdstanr and CmdStan 2.39.0 (PROG-1 closed). The first real fit sampled cleanly, which confirms the C1 fix, and then exposed three defects no reading could have found: extraction indexed the sampler diagnostics as a matrix (they are a three-dimensional draws array) and crashed; the convergence check's divergence count tested `colnames()` of that array, which is NULL, so it never ran; and the model file was looked up relative to the working directory only. All three fixed in the next commit. Executed after the fix: Stan HB on 60 simulated respondents recovers known utilities (Spearman rho 1.0), zeroes the designated anchor with the columns back in configured order, fills `HB_Mean_SE`, 0 divergences, Rhat 1.000, min ESS 1109; the synthetic end-to-end run with `Generate_HB_Model = YES` completes on Stan in 82 seconds with MODEL_DIAGNOSTICS, the stats pack and the report all saying Stan. A recovery test for the Stan path is in the suite, gated on CmdStan being present. Suite 963 / 0 / 1 skip / 0.
