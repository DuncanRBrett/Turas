...
**Fixes committed by this review**, on `feature/segment-demographics`, not merged, not pushed: `69d29a94` (D1, the base wording, with a test), `d11aabd6` (D5, a test for the sheet-name collision guard), `ec5b14f0` (D6, the template description), and, after Duncan's rulings on 22 September, `8f378775` (D10, `tabs_export` refused in exploration mode) and `f29cb13d` (D9, a constant demographic no longer tested or announced). This document is committed on the same branch from `140e1587`. D2, D3, D4, D7 and D8 remain open, because each changes what a reader sees or what `profile_demographics()` returns.
...
**Ruled on and closed, 22 September.** Duncan ruled on D9 and D10 after reading this review, and both are fixed on the branch with tests watched failing first. A constant demographic is no longer tested or announced, and `tabs_export = Y` is refused in exploration mode as it already was in combined mode. The shipped Thornhill example is untouched by either: same three significant demographics, and the percentages still match my independent recompute to 0.0000.

Suite on the branch before my fixes: FAIL 0, WARN 0, SKIP 0, PASS 1345, from my own run. After my first three fixes: FAIL 0, WARN 0, SKIP 0, PASS 1355. After the D9 and D10 fixes: FAIL 0, WARN 0, SKIP 0, PASS 1392. At `db2c66dc`: FAIL 0, WARN 0, SKIP 0, PASS 1258, which matches the handover's stated baseline exactly.
...
Duncan ruled on D9 and D10 on 22 September and both are fixed on the branch; what follows is what is still open.

1. **D1, the answered n.** The wording is fixed; the number is not. Saying which base each table rests on needs `profile_demographics()` to return an answered n per variable, which changes its contract.
