...
**Fixes committed by this review**, on `feature/segment-demographics`, not merged, not pushed: `69d29a94` (D1, the base wording, with a test), `d11aabd6` (D5, a test for the sheet-name collision guard), `ec5b14f0` (D6, the template description). This document is committed on the same branch at `140e1587` and amended by a later commit. D2, D3, D4, D7, D8 and D9 are left for Duncan, because each changes what a reader sees or what `profile_demographics()` returns.
...
Suite on the branch before my fixes: FAIL 0, WARN 0, SKIP 0, PASS 1345, from my own run. After them: FAIL 0, WARN 0, SKIP 0, PASS 1355. At `db2c66dc`: FAIL 0, WARN 0, SKIP 0, PASS 1258, which matches the handover's stated baseline exactly.
...
1. **D9, a constant demographic reported as significant.** The loudest of these, because it puts a fabricated significant result into a client-facing sheet and onto the console. Either skip the chi-square for a one-row table and say so in the sheet, or refuse a constant demographic at load the way a missing one is refused.
2. **D1, the answered n.** The wording is fixed; the number is not. Saying which base each table rests on needs `profile_demographics()` to return an answered n per variable, which changes its contract.
3. **D10, `tabs_export` in exploration mode.** The same ruling F14 got, applied to the third path. Refusing it there too would make the setting mean one thing everywhere.
4. **D2, a demographic of an unhandled class.** Refuse it at load, route it to the categorical branch, or name it in the skipped note. Refusing at load is the most consistent with what Session D built for the missing-name case.
5. **D3, NA in the ten-value threshold.** Deciding this changes what a ten-point coded demographic with blanks looks like in the report, so it is a judgement about what a reader should see, not a bug fix.
6. **launch_turas() eyeball.** Still owed on this branch, per the handover's ground rules. Nothing in this review substitutes for it.
