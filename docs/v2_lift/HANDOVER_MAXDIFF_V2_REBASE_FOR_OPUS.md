...
| `examples/maxdiff/*` | copied verbatim from the v2 branch (`c35396b9`), minus `Output/`; then Duncan set `Generate_HTML_Report = YES` in `Karoo_MaxDiff_Config.xlsx` in Excel | the originals | All identical except the config workbook: take THIS branch's `Karoo_MaxDiff_Config.xlsx` (report on). Do not merge the binary by hand. |
