...
The author's figures were 278 / 919 / 0 / 1 and 469 / 1,330 / 0; mine differ by two and twenty-three expectations, which is within what a stubbed stats-pack writer and platform-dependent minify checks move between runs. The one surviving catdriver warning is a raw `glm()` call at `test_catdriver.R:873`, not the engine.
...
`coord/anova_weights.R`: on a weighted binomial glm fitted exactly as `04_analysis.R` fits it (local `fit_data`, `weights = .wt`), `car::Anova(type = "II")` reports 50.920 for service_quality; my manual weighted deviance difference between the full and the reduced weighted fits is 50.92049; the unweighted difference is 50.287 and the Wald chi-square is 44.06. Anova's refits carry the weights, and the statistic is the likelihood-ratio one, so the new stamp is right and the old hardcoded "Type II Wald" was wrong.
...
- branch: `or_comparison` 9 rows x 17 columns, `importance_matrix` 4 x 14 with a `classification` column (one Universal, three Mixed), `model_fit` 4 rows, two insights, "[OK] Subgroup comparison sheets added". Cell-level agreement with an independently built comparison is Reader C's; see section 4.
...
- Readers A, B and C were cut by an account rate limit before writing their reports and replied from their transcripts afterwards; every number they report was produced in their session and is on disk under the scratchpad, but the coordinator re-ran only the items marked "coordinator" above.

## 6. Verdict

**PASS WITH FIXES.** 0 CRITICAL, 5 HIGH, 16 MEDIUM, 9 LOW, 3 OBSERVATIONS.
...
What stops it being a plain PASS is that this session's own subject was provenance, and five HIGHs are provenance defects the session introduced or left: an ordinal importance stamp that names the wrong statistic (F1), a weight variable that does not exist running unweighted under PASS with a Declaration that says weighted (F2), a multinomial subgroup table that overwrites outcome levels (F3), an ordinal "lift" that is not one, now with a label and a callout that make it read as defined (F4), and a Declaration whose two respondent counts are swapped (F5). None of these needs a design decision; F1, F2 and F5 are each a few lines. F3 and F4 are on paths the July review did not reach and deserve their own tests. The MEDIUMs are mostly one pattern: the branch put the right words into channels (guard warnings, `proportional_odds`, `method`, `outcome_level`, the stats-pack event `problem`) that no sheet reads.

Recommended order before Duncan's `launch_turas()` eyeball: F1, F2, F5, F6 (an afternoon, all with tests that fail on the current branch), then F3 and F4, then the test gaps F18 to F21 so the next session cannot regress H1 in silence.
