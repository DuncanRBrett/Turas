...
calculate_weighted_base <- function(data_subset, question_info, weights) {
  ...
  if (question_info$Variable_Type == "Multi_Mention") {
    ...
    # V9.9.6: Handle Ranking questions
    # Ranking questions have multiple columns depending on format:
    # - Item format: Q_Rank1, Q_Rank2, Q_Rank3, etc.
    # - Position format: Q_ItemA, Q_ItemB, Q_ItemC, etc.
...
if (is.numeric(col_values)) {
  ...
  # V9.9.2: No rounding (full precision for upstream calculations)
  return(list(
    unweighted = unweighted_n,
    weighted = weighted_n,  # Full precision, no rounding
...
weighted_t_test_means <- function(values1, values2, 
  ...
  # V9.9.3: Validate lengths (fail fast)
  ...
  # V9.9.2 FIX: Filter to analytic sample FIRST, then compute eff-n
  ...
  tryCatch({
    ...
    # Calculate weighted variances (V9.9.1: uses weighted_variance function)
...
chi_square_test <- function(observed_matrix, min_expected = 5, alpha = 0.05) {
  ...
  if (nrow(observed_matrix) < 2 || ncol(observed_matrix) < 2) {
    return(list(
      significant = FALSE,
      p_value = NA_real_,
      chi_square_stat = NA_real_,
      df = NA_integer_,
      warning = "Insufficient dimensions (need at least 2×2 table)"
    ))
  }
  ...
  if (any(is.na(observed_matrix))) {
    return(list(
      ...
      chi_square_stat = NA_real_,
      df = NA_integer_,
      warning = "Matrix contains NA values"
    ))
  }
  ...
  if (any(observed_matrix < 0)) {
    return(list(
      ...
      chi_square_stat = NA_real_,
      df = NA_integer_,
      warning = "Matrix contains negative values"
    ))
  }
  ...
  tryCatch({
    ...
    return(list(
      ...
      chi_square_stat = NA_real_,
      df = NA_integer_,
      warning = sprintf("Chi-square test failed: %s", conditionMessage(e))
    ))
  })
}
