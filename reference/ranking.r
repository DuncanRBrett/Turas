...
validate_ranking_matrix <- function(ranking_matrix, num_positions, item_names = NULL,
  ...
  # V9.9.2: Vectorized tie detection (apply-based, faster)
  ...
  # V9.9.2: Vectorized gap detection (apply-based, faster)
  ...
  # Compile issues (V9.9.2: uses configurable thresholds)
  ...
  if (non_integer > 0) {
    issues <- c(issues, sprintf("%d non-integer rank values", non_integer))
  }
...
extract_ranking_data <- function(data, question_info, option_info, config = NULL) {
  ...
  # V9.9.2: Normalize rank direction if needed
  ...
  # V9.9.2: Get validation thresholds from config
  tie_threshold <- 5
  gap_threshold <- 5
...
create_ranking_rows_for_item <- function(ranking_matrix, item_name, banner_data_list,
  ...
  for (key in internal_keys) {
    ...
    if (any(subset_idx < 1 | subset_idx > nrow(ranking_matrix))) {
      warning(sprintf(
        "Invalid row indices for banner %s, skipping",
        key
      ), call. = FALSE)
      pct_first_row[[key]] <- NA
      mean_rank_row[[key]] <- NA
    ...
    if (length(subset_weights) != length(subset_idx)) {
      warning(sprintf(
        "Weight length mismatch for banner %s, using unweighted",
        key
      ), call. = FALSE)
    ...
    tryCatch({
      ...
      # V9.9.2: Named args in format_output_value
      pct_first_row[[key]] <- if (exists("format_output_value", mode = "function")) {
        format_output_value(
          first_result$percentage, 
          "percent",
        ...
        if (is.na(first_result$percentage)) NA else round(first_result$percentage, decimal_places_percent)
      }
      ...
      # V9.9.2: Named args in format_output_value
      mean_rank_row[[key]] <- if (exists("format_output_value", mode = "function")) {
        format_output_value(
          mean_rank, 
          "index",
        ...
        if (is.na(mean_rank)) NA else round(mean_rank, decimal_places_index)
      }
      ...
      if (show_top_n) {
        ...
        # V9.9.2: Named args in format_output_value
        top_n_row[[key]] <- if (exists("format_output_value", mode = "function")) {
          format_output_value(
            top_n_result$percentage, 
            "percent",
            decimal_places_percent = decimal_places_percent,
            decimal_places_ratings = NULL,
            decimal_places_index = NULL
          ...
          if (is.na(top_n_result$percentage)) NA else round(top_n_result$percentage, decimal_places_percent)
        }
      }
    }, error = function(e) {
      warning(sprintf(
...
validate_ranking_question <- function(question_info, options_info, error_log) {
  ...
  # Check Ranking_Positions exists
  has_positions <- FALSE
