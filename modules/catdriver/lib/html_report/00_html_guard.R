# ==============================================================================
# CATDRIVER HTML REPORT - GUARD LAYER
# ==============================================================================
...
#' Validate Inputs for Catdriver HTML Report
#'
...
#' @keywords internal
validate_catdriver_html_inputs <- function(results, config, output_path) {
  ...
  if (missing(results) || is.null(results) || !is.list(results)) {
    return(list(
      ...
      how_to_fix = "Provide analysis results from run_categorical_keydriver()"
    ))
  }

  # Check required result components
  required_fields <- c("importance", "factor_patterns", "model_result",
                        "odds_ratios", "diagnostics", "prep_data")
  ...
  if (missing(config) || is.null(config) || !is.list(config)) {
    return(list(
      ...
      how_to_fix = "Provide configuration list from load_catdriver_config()"
    ))
  }
  ...
  list(status = "PASS", message = "Catdriver HTML report inputs validated successfully")
}
