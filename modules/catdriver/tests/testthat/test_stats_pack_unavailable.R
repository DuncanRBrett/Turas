...
test_that("generate_catdriver_stats_pack warns loudly when the writer is missing", {
  skip_if(!exists("generate_catdriver_stats_pack", mode = "function"),
          "generate_catdriver_stats_pack not available")
  ...
  out <- capture.output(
    result <- generate_catdriver_stats_pack(NULL, NULL, NULL, NULL, Sys.time(),
                                            verbose = FALSE)
  )
