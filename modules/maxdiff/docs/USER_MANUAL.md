...
3. [Configuration Workbook](#3-configuration-workbook)
...
## 3. Configuration Workbook

The module is controlled by a single Excel workbook (.xlsx) containing up to six sheets.

### 3.1 PROJECT_SETTINGS (Required)
...
### 3.3 DESIGN_SETTINGS (Required for DESIGN mode)
...
### 3.4 SURVEY_MAPPING (Required for ANALYSIS mode)

Maps your survey column names to the module's expectations:
...
- `BEST_CHOICE`: Column containing the best item choice (as Item_ID or position 1-k)
...
### 3.5 SEGMENT_SETTINGS (Optional)

Define segments for subgroup analysis:

| Segment_ID | Segment_Label | Variable_Name | Segment_Def | Include_in_Output |
|------------|---------------|---------------|-------------|-------------------|
| `GENDER` | `Gender` | `Gender` | | 1 |
| `AGE3` | `Age Groups` | `Age` | `cut(Age,c(0,29,44,120),labels=c("18-29","30-44","45+"))` | 1 |
| `REGION` | `Region` | `Region` | | 1 |

- Leave `Segment_Def` blank to use raw categories from `Variable_Name`
- Use R expressions in `Segment_Def` to create custom groupings

### 3.6 OUTPUT_SETTINGS (Optional)
...
- One column per item
- Can merge with other respondent data
...
| `*_utility_distribution.png` | Violin plot of individual utilities |
...
In SEGMENT_SETTINGS, you can define segments two ways:

**Simple categorical:**
```
Variable_Name: Gender
Segment_Def: (leave blank)
```
Uses existing categories in the data.

**Custom groupings using R expressions:**
```
Variable_Name: Age
Segment_Def: cut(Age, c(0,29,44,120), labels=c("18-29","30-44","45+"))
```
...
|---------|-------|----------|
| "Respondent chose same item for best and worst" | Data error | Check survey logic |
...
3. Consult the Technical Reference for advanced issues
...
- **Items:** 8-15 items is ideal; more requires careful design
- **Tasks:** 10-15 tasks per respondent balances burden and precision
- **Items per task:** 4-5 is standard
- **Versions:** 2-4 versions improves design efficiency

### 9.2 Configuration

- Always set a `Seed` for reproducibility
- Use descriptive `Item_ID` values (e.g., `PRICE_LOW` not `I1`)
- Test with a small sample before full analysis
- Keep your config file with your project files

### 9.3 Analysis

- Review count scores first for sanity check
- Compare logit and HB results - they should be similar
- Check segment sizes before interpreting differences
- Export individual utilities for advanced segmentation

### 9.4 Reporting

- Use 0-100 rescaled scores for client presentations
- Include confidence intervals for key comparisons
- Show both utility bars and best-worst diverging charts
...
| Term | Definition |
|------|------------|
| **Anchor Item** | Reference item with utility fixed at 0 in HB model |
| **Best-Worst Scaling** | Alternative name for MaxDiff |
| **BIBD** | Balanced Incomplete Block Design |
| **Conditional Logit** | Statistical model for discrete choice data |
| **D-efficiency** | Measure of design quality (higher = better) |
| **Hierarchical Bayes** | Bayesian method for individual-level estimation |
...
| **Version** | One of multiple design variants |
