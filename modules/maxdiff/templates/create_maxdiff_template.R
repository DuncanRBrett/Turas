...
create_maxdiff_template <- function(output_path = NULL) {
  ...
  output_settings <- data.frame(
    ...
    Description = c(
      ...
      "",
      "Write each respondent's preference shares as a tabs Allocation question ({output}_tabs_shares.xlsx), so MaxDiff results can be crosstabbed by banner. Needs Generate_HB_Model = YES.",
      "QuestionCode for the exported shares; columns are {code}_1 .. {code}_k",
      "Let the count-based empirical-Bayes fallback (no cmdstanr) through the tabs export. The QuestionText and METHOD sheet are then stamped 'approximate: count-based'. Genuine Stan HB estimates never need this.",
      "",
    ...
    Options = c(
      ...
      "YES or NO",
      "",
      "YES or NO (requires Generate_HB_Model = YES)",
      "Letters, digits, underscores (default MDSHARE)",
      "YES or NO (default NO: approximate utilities refuse)",
      "",
  ...
  yn_setting_names <- c("Generate_Design_File", "Generate_Count_Scores",
                        ...
                        "Generate_Stats_Pack",
                        "Generate_Tabs_Export", "Allow_Approx_Utilities_Export",
