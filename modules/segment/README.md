...
- **11.3** (2026-09): Latent class analysis and ensemble clustering REMOVED. Both were config-exposed and documented but unreachable from any production path, so a config asking for either silently got k-means. A config that still sets `use_lca = TRUE` or `method = ensemble` is now refused by name.
