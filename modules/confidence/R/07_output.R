...
add_methodology_sheet <- function(wb) {
  ...
  methodology_text <- c(
    ...
    "   with samples of 100+.",
    "   Limitation: Poor coverage for extreme proportions; the bounds are",
    "   cut off at 0% and 100%, and at exactly 0% or 100% the interval has",
    "   zero width. Not reliable when n*p < 10.",
    ...
    "   Technical: Posterior = Beta(alpha + x, beta + n - x), with a",
    "   Beta(1, 1) prior when none is set. Weighted: n = n_eff, x = p * n_eff.",
    "",
    "----------------------------------------------------------------------",
    "",
    ...
    "   and df = n_eff - 1. Below an effective n of 2 no t interval is given.",
    "",
    "2. Bootstrap",
    ...
    "for simple proportions. Promoters and detractors come from the same",
    "respondents, so the standard error includes their covariance:",
    "  SE = 100 * sqrt((p_p + p_d - (p_p - p_d)^2) / n_eff)",
    "",
    "----------------------------------------------------------------------",
    "",
    ...
    "",
    "  DEFF = n / n_eff = 1 + CV^2 (CV = coefficient of variation of the",
    "  weights, population standard deviation)",
