...
build_sampling_note <- function(sampling_method = "Not_Specified") {
  note <- switch(sampling_method,
    ...
    "Stratified" = '<strong>Sampling design: Stratified random sampling.</strong> The population was divided into groups (e.g. by channel, region, or segment), and individuals were sampled randomly within each group. This approach improves the ability to generate reliable estimates for each group, including smaller ones, provided sufficient sample sizes are achieved, but may involve deliberate oversampling relative to true population proportions. Within each group, confidence intervals are statistically valid under standard sampling assumptions, although precision varies by sample size. At the total level, results should be interpreted with care if disproportionate sampling has been used. Weighting can be applied to restore population representation, but this typically increases variance (design effect), which may widen confidence intervals. As with any survey, results may be affected by nonresponse bias if those who declined to participate differ systematically from those who took part.',
