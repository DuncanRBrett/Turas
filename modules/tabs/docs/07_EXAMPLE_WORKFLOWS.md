...
This document walks through complete examples of common analysis scenarios. Each workflow shows the configuration files, data setup, and expected output.
...
You've conducted a brand tracking survey with 500 respondents. You want to analyze brand awareness and preference, broken out by gender and age group.
...
Looking at this output, you can see that Brand A has 40% total awareness, but awareness differs by demographics - Males (44%) are significantly more aware than Females (36%). The younger age group (18-34) has the highest Brand A awareness at 45%.
...
You've surveyed 1,000 customers about their satisfaction. You want to measure overall satisfaction on a 5-point scale and calculate Net Promoter Score, broken out by customer segment.
...
Enterprise customers are significantly more satisfied than SMB and Consumer segments. The NPS score shows this clearly: Enterprise has +40, while Consumer has -7.
...
Your sample over-represents urban respondents. You need to apply weights to match the population distribution.
...
Urban respondents have weights less than 1 (over-sampled), Rural respondents have weights greater than 1 (under-sampled).
...
The unweighted base (2000) is your actual sample count. The weighted base (2000) is the population estimate (sum of weights). The effective N (1538) accounts for weighting variance and tells you the true statistical precision of your sample.

The DEFF of 1.30 means your sample is worth about 77% of its nominal size for statistical purposes (2000 / 1.30 = 1538).
...
You want to analyze brand preference across multiple demographic cuts simultaneously: gender, age group, and income.
...
This creates a banner with 10+ columns (Total, Male, Female, 18-34, 35-54, 55+, Low Income, Medium Income, High Income).
...
Brand A is preferred by males, younger consumers, and higher-income groups. Brand B shows the opposite pattern - preferred by females and lower-income groups. Regional analysis shows no significant differences.
...
You want to analyze satisfaction only among customers who made a purchase, and service quality only among those who contacted support.
...
The output clearly shows that only 800 of 1500 respondents are included in this analysis - those who answered "Yes" to the purchase question.
...
You have a product evaluation survey with multiple 1-5 rating scales. You want to show both the distribution and summary metrics like Top 2 Box.
...
You asked respondents "Which features do you use?" with a select-all-that-apply format.

### Step 1: Data Structure

Your data has columns Q10_1 through Q10_5, each containing a feature name if selected:
...
Notice that percentages sum to more than 100% - this is expected for multi-mention questions since each respondent can select multiple features.
...
You want to calculate an Overall Satisfaction Index by averaging three individual satisfaction questions.
...
You have a large dataset (50,000+ respondents, 200+ questions) and need to process efficiently.
...
Disable frequencies if you only need percentages - this reduces output size and processing time.
...
Values indicate the rank position each brand received (1=first choice, 2=second, etc.).
...
The Index_Summary sheet becomes your executive dashboard with all scores across segments.
...
- Differences genuinely not significant
- Alpha level too strict

**Solutions:**
- Check base sizes (need 30+ per column for reliable testing)
- Try alpha = 0.10 for more lenient testing
- Verify you're using the appropriate statistical test
