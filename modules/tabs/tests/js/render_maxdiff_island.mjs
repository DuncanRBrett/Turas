#!/usr/bin/env node
/**
 * Render a MaxDiff island through the SHIPPED tab JS and print the HTML.
 *
 * A helper, not a gate. It exists so an R test can prove the seam between the
 * two languages: 13_v2_island.R writes the JSON, this feeds that exact file to
 * 27y_maxdiff.js, and the R test asserts on what came out. Both sides have
 * their own tests; only this proves they agree about the shape.
 *
 * Usage:  node modules/tabs/tests/js/render_maxdiff_island.mjs <island.json>
 */
...
if (!file) { console.error("usage: render_maxdiff_island.mjs <island.json>"); process.exit(2); }
sandbox.TR.MD = JSON.parse(readFileSync(file, "utf8"));
