...
# ==============================================================================
# All colour palette, style factories, write_settings_sheet(), and
# write_table_sheet() come from the shared module.

# Skip if already sourced (e.g., from test harness)
if (!exists("write_settings_sheet", mode = "function")) {
  shared_path <- tryCatch({
    file.path(dirname(dirname(dirname(sys.frame(1)$ofile))), "shared", "template_styles.R")
  }, error = function(e) "")
  ...
  if (!file.exists(shared_path)) {
    stop("Cannot locate modules/shared/template_styles.R", call. = FALSE)
  }
  source(shared_path)
}
...
generate_crosstab_config_template <- function(output_path,
  ...
  settings_def <- list(
    ...
    list(
      ...
      fields = list(
        ...
        list(name = "min_reporting_base", default = 1, required = FALSE,
             description = paste("Disclosure control: minimum audience below which the report hides",
               "identifying detail (comment demographic tags; small cells) so a filter cannot narrow",
               "onto identifiable people. 1 = off. Use ~10 for a small/sensitive sample (e.g. a staff",
               "climate survey); set it to the full sample size to forbid any sub-group drill-down.",
               "NOTE: this gates the SCREEN; for a k-gate that must hold in the shipped FILE, also set",
               "html_report_v2_microdata = FALSE (the confidential ship)."),
             valid_values_text = "1 to 100000 (1 = off; typical sensitive: 10)",
             integer_range = c(1, 100000)),
    ...
    list(
      ...
      fields = list(
        ...
        list(name = "accent_colour", default = "#CC9900", required = TRUE,
             description = "Accent colour for HTML report highlights and call-outs. Also used as the default crosstab heatmap tint colour.",
             valid_values_text = "Hex colour code (e.g. #CC9900)"),
        list(name = "heatmap_colour", default = "", required = FALSE,
             description = "Crosstab heatmap cell tint colour. Defaults to accent_colour (warm cream for standard #CC9900). Set explicitly to override without affecting other colours (e.g. #323367 for cool blue).",
             valid_values_text = "Hex colour code, or leave blank to use accent_colour"),
        list(name = "chart_bar_colour", default = "#323367", required = TRUE,
             description = "Colour for bar charts in the HTML report.",
             valid_values_text = "Hex colour code (e.g. #323367)"),
        list(name = "chart_palette_preset", default = "warm", required = TRUE,
             description = "Colour palette for stacked bar charts. warm = earth tones, cool = blue-anchored, research = purple-green diverging, teal = monochromatic teal gradient, red = Coca-Cola-inspired muted red gradient, brand = monochromatic gradient from brand_colour.",
        ...
        list(name = "chart_series_colour_2", default = "", required = FALSE,
             ...
             valid_values_text = "Hex colour code (e.g. #3A6EA5)"),
        list(name = "chart_series_colour_3", default = "", required = FALSE,
             description = "Custom colour for 3rd banner series in bar charts. Leave blank to auto-generate.",
             valid_values_text = "Hex colour code (e.g. #E87722)"),
        list(name = "chart_series_colour_4", default = "", required = FALSE,
             description = "Custom colour for 4th banner series in bar charts. Leave blank to auto-generate.",
             valid_values_text = "Hex colour code (e.g. #5B9A7D)"),
        list(name = "chart_series_colour_5", default = "", required = FALSE,
             description = "Custom colour for 5th banner series in bar charts. Leave blank to auto-generate.",
             valid_values_text = "Hex colour code (e.g. #8E4585)"),
        list(name = "chart_series_colour_6", default = "", required = FALSE,
             description = "Custom colour for 6th banner series in bar charts. Leave blank to auto-generate.",
             valid_values_text = "Hex colour code (e.g. #C14D00)"),
        list(name = "chart_series_colour_7", default = "", required = FALSE,
             description = "Custom colour for 7th banner series in bar charts. Leave blank to auto-generate.",
             valid_values_text = "Hex colour code (e.g. #4A7C6F)"),
        list(name = "chart_series_colour_8", default = "", required = FALSE,
             description = "Custom colour for 8th banner series in bar charts. Leave blank to auto-generate. If all 8 set and more than 8 series exist, colours cycle.",
             valid_values_text = "Hex colour code (e.g. #D4918E)"),
      ...
      fields = list(
        ...
        list(name = "html_report_v2_microdata", default = "TRUE", required = FALSE,
             description = paste0("Embed the anonymised per-respondent microdata island in the v2 report ",
               ...
               "Tracking tab still works (its current wave is built from the published figures), ",
               "but no *_wave.json is written - keep the one from your microdata run. ",
               "FALSE is REQUIRED wherever min_reporting_base is a promise - with the ",
               "island present, sub-k detail is reconstructable from the page source. Tip: keep TRUE ",
               "for your own working copy and build the client copy with FALSE from a second config."),
             valid_values_text = "TRUE or FALSE",
             dropdown = c("TRUE", "FALSE")),
        list(name = "html_report_v2_interactivity", default = "records", required = FALSE,
             description = paste0("What powers the v2 report's LIVE views. 'records' embeds the ",
               "de-identified per-respondent island, coded answer indices and weights: every ",
               "feature works. 'cube' embeds ",
               "precomputed group statistics instead: the live filter, custom and composite banners, ",
               "Differences, Pattern Recognition, confidence detail and the Reader all still work, on ",
               "the DECLARED variables only, and the file carries no respondent-level records at all. ",
               "'none' is published tables with no live views, which is what html_report_v2_microdata ",
               "= FALSE has always meant. 'cube' REQUIRES min_reporting_base above 1: it withholds any ",
               "cut in which a group falls below it. Leave blank to inherit from ",
               "html_report_v2_microdata; setting both and disagreeing is refused."),
             valid_values_text = "records, cube or none",
             dropdown = c("records", "cube", "none")),
        list(name = "html_report_v2_filter_vars", default = "", required = FALSE,
             description = paste0("Extra questions a 'cube' build may be filtered and cross-tabbed by, ",
               "beyond the banner groups, comma separated (e.g. Q008,Q012). Each must be a ",
               "SINGLE-RESPONSE question with category rows: a multi-mention answer is a set rather ",
               "than a partition and is not supported in this version. Declare what you know the ",
               "client will want to cut by. A cut by anything undeclared is refused on the page, ",
               "which is the point: the client cannot invent a cut the report was not built to ",
               "answer. Ignored on 'records' and 'none' builds."),
             valid_values_text = "Comma-separated question codes, or leave blank"),
        list(name = "html_report_v2_cube_order", default = "2", required = FALSE,
             description = paste0("How many declared variables a live filter on a 'cube' build may ",
               "combine. One filter plus a banner is TWO, so 2 is what 'filter and still see every ",
               "banner' costs, and it is the default. 3 allows a filter on two variables while still ",
               "showing a banner, at roughly a third more file size and more withheld cuts on a small ",
               "sample. 1 is filter-or-banner, not both. Ignored on 'records' and 'none' builds."),
             valid_values_text = "1, 2 or 3",
             integer_range = c(1, 3),
             dropdown = c("1", "2", "3")),
        ...
        list(name = "html_report_v2_cover", default = "FALSE", required = FALSE,
             description = paste0("Open SAVED copies of the v2 report on a cover page: report title, the ",
               "executive summary (the first narrative screen whose heading starts \"Executive summary\", ",
               "else the first screen), and the first five Story pins as leading findings. Only affects ",
               "copies made with Save copy, and only when there is story content to show. FALSE (the ",
               "default) = saved copies open on the dashboard, as before."),
             valid_values_text = "TRUE or FALSE",
             dropdown = c("TRUE", "FALSE")),
        list(name = "html_report_v2_cover_findings", default = "", required = FALSE,
             description = paste0("How many Story pins the cover lists as leading findings. ",
               "Blank = 5. ALL = every pin, however many. When the cover does stop short it ",
               "says so on the page (\"Showing 5 of 12 pinned findings\"). Note the exported ",
               "PowerPoint cover is a single slide and always shows the first 5."),
             valid_values_text = "Whole number of 1 or more, ALL, or leave blank for 5"),
        list(name = "narrative_file", default = "", required = FALSE,
             description = paste0("Word document (.docx) holding the report's Background and ",
               "Executive summary. Each Heading 1 starts a screen, with Heading 2 and 3, paragraphs, ",
               "bold, italic, coloured and highlighted words (shown in the report's own colours), ",
               "lists, Quote style, pictures and simple tables inside it. ",
               "Anything else in the document is skipped and named in the console. The path ",
               "is relative to this config file. Each screen shows on the Report tab and can be ",
               "pinned to the Story. Blank = the Comments sheet's _BACKGROUND and ",
               "_EXECUTIVE_SUMMARY cells are used, as before."),
             valid_values_text = "Path to a .docx file, or leave blank"),
        ...
        list(name = "conjoint_island", default = "", required = FALSE,
             ...
             valid_values_text = "Path relative to this config file, or a full path, or leave blank"),
        list(name = "maxdiff_island", default = "", required = FALSE,
             description = paste0("Path to a MaxDiff study's contribution file for this project ",
        ...
        list(name = "pricing_island", default = "", required = FALSE,
             ...
             valid_values_text = "Path relative to this config file, or a full path, or leave blank"),
        list(name = "keydriver_island", default = "", required = FALSE,
             description = paste0("Path to a key driver study's contribution file for this ",
               "project ({output}_kd_island.json, written by the keydriver module). Names it ",
               "and the report gains a Key drivers tab showing importance per driver by method ",
               "with bootstrap intervals, the importance-performance quadrant, the driver by ",
               "segment matrix with its bases, and the model diagnostics. Blank -> no Key ",
               "drivers tab, and nothing else changes."),
             valid_values_text = "Path relative to this config file, or a full path, or leave blank"),
        list(name = "catdriver_island", default = "", required = FALSE,
             description = paste0("Path to a categorical driver study's contribution file for ",
               "this project ({output}_cd_island.json, written by the catdriver module when its ",
               "v2_island setting is on). Names it and the report gains a Categorical drivers ",
               "tab showing driver importance, the odds ratio for every driver level with its ",
               "interval, probability lifts for a named outcome level, the raw factor patterns, ",
               "any subgroup comparison, and the model diagnostics. Blank -> no Categorical ",
               "drivers tab, and nothing else changes."),
             valid_values_text = "Path relative to this config file, or a full path, or leave blank"),
        list(name = "whatif_island", default = "", required = FALSE,
             description = paste0("Path to a What if study's contribution file for this project ",
               "({output_name}_whatif_island.json, written by the whatif module). Names it and the ",
               "report gains a What if tab: where to direct effort, scenarios and Build a ... . ",
               "A full report gets the live version that follows the filter bar; a client-safe ",
               "report gets precomputed results for published groups only. Blank -> no What if ",
    ...
    list(
      ...
      fields = list(
        ...
        list(name = "show_qualitative", default = "TRUE", required = FALSE,
             ...
             valid_values_text = "TRUE or FALSE",
             dropdown = c("TRUE", "FALSE")),
        list(name = "show_save_copy", default = "TRUE", required = FALSE,
             description = paste0("Show the Save copy button in the v2 report header. Set FALSE for a ",
               "copy that is published rather than worked in, such as a public demo or a client ",
               "copy that must stay the one version of record."),
             valid_values_text = "TRUE or FALSE",
             dropdown = c("TRUE", "FALSE"))
      )
    ),
    ...
    list(
      ...
      fields = list(
        ...
        list(name = "generate_stats_pack", default = "Y", required = FALSE,
             description = "Generate a diagnostic stats pack workbook alongside main output. Stats packs are contractual deliverables. Output file is named {output}_stats_pack.xlsx.",
    ...
    list(
      ...
      fields = list(
        ...
        list(name = "qual_verbatim_scope", default = "all", required = FALSE,
             description = paste0(
               "Which comments ship their readable text (a BUILD-TIME curation, not a reader ",
  ...
  selection_cols <- list(
    ...
    list(name = "Category", width = 20, required = FALSE,
         description = "Report section this question belongs to (e.g. 'Service', 'Delivery'). Groups questions in the v2 report's navigation and dashboards. Blank = uncategorised."),
    ...
    list(name = "ExcludeFromInsights", width = 20, required = FALSE,
         description = paste("Y = keep this question OUT of the Differences tab's findings, while it stays",
           "in the crosstabs as normal. Use it when several near-duplicate views of one measure",
           "(Own / Other / Total) each raise their own card, or when a modelled figure should not",
           "lead a lay reader's page. Blank = the question can raise findings. The Group overview",
           "scan ignores this column by design - it is a Differences-tab lever only."),
         dropdown = c("Y", "N")),
    list(name = "Source", width = 30, required = FALSE,
         description = paste("Where this question's numbers come from - the survey question behind it, or",
           "the columns a derived figure was built from. Shown on the question card in the v2 report.",
           "Blank = the report says nothing about provenance.")),
    list(name = "Formula", width = 40, required = FALSE,
         description = paste("Derived questions only: how the figure was worked out, in plain words",
           "(e.g. 'monthly spend / monthly transactions, among buyers'). A question with a formula reads",
           "as DERIVED in the report; one with a Source and no formula reads as ASKED. Fill this in for",
           "any column computed before the data reached Turas - the engine sees only the finished column.")),
    ...
    list(name = "SplitDimension", width = 14, required = FALSE,
         description = "Band-split open-ends only: the name of the split axis (default 'NPS band'). Used as the segmented-view label and the export column header."),
    list(name = "NpsScoreQuestion", width = 16, required = FALSE,
         description = "Band-split open-ends only: the 0-10 recommend question to DERIVE each comment's band from (the score wins over which sheet the text landed in). Defaults to the CommentLink target."),
  ...
  selection_examples <- list(
    ...
    list(QuestionCode = "Q_OpenEnd", Include = "N", UseBanner = "N",
         ...
         SplitDimension = "", NpsScoreQuestion = ""),
    list(QuestionCode = "Q_Recommend", Include = "Y", UseBanner = "N",
         BannerBoxCategory = "", BannerLabel = "",
         DisplayOrder = "", CreateIndex = "N", BaseFilter = "",
         FilterLabel = "",
         QuestionText = "Example: NPS 'why?' split across Detractor/Passive/Promoter sheets into one question",
         CommentSheet = "DetractorComment:Detractor; PassiveComment:Passive; PromoterComment:Promoter",
         CommentLink = "Q_Recommend", SplitDimension = "NPS band", NpsScoreQuestion = "Q_Recommend")
  )
  ...
  comments_cols <- list(
    list(name = "QuestionCode", width = 20, required = TRUE,
         description = paste0(
           ...
           "_REPORT_CONSTRUCTION for report sections (_BACKGROUND and _EXECUTIVE_SUMMARY ",
           "are used only when the Settings sheet's narrative_file is blank). A QUAL_ code targets the ",
    ...
    list(name = "Comment", width = 60, required = TRUE,
         description = paste0("Comment text. Supports markdown for HTML reports (bold, bullets, links). ",
           "In _BACKGROUND and _EXECUTIVE_SUMMARY each line is its own paragraph and a line ",
           "starting \"- \" or \"* \" is a bullet; nothing else is formatted.")),
  ...
  write_table_sheet(wb, "Comments", comments_cols,
                    ...
                    subtitle = paste("Add question-specific comments and report narrative sections.",
                      "Use _BACKGROUND, _EXECUTIVE_SUMMARY and _REPORT_CONSTRUCTION for report sections.",
                      "A Word document named in the Settings sheet's narrative_file replaces",
                      "_BACKGROUND and _EXECUTIVE_SUMMARY."),
...
generate_survey_structure_template <- function(output_path) {
  ...
  options_cols <- list(
    ...
    list(name = "ShowInOutput", width = 14, required = TRUE,
         description = "Show this option in output tables. Set to Y to include, N to hide (e.g. internal codes).",
         dropdown = c("Y", "N")),
    ...
    list(name = "Index_Weight", width = 14, required = FALSE,
         description = "Numeric weight for the Likert index (e.g. -100 to 100). Every option needs one; with none, no index row is written. Rating means don't use it - see OptionValue."),
    list(name = "OptionValue", width = 14, required = FALSE,
         description = "Numeric value used for calculations instead of OptionText. Scores Rating means and box/NET ordering; leave blank when OptionText is already the number."),
