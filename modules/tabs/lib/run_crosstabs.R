...
#' Read a CatDriver Contribution For This Project
#'
#' The catdriver module writes `{output}_cd_island.json` when its `v2_island`
#' setting is on. A tabs run embeds it when `catdriver_island` names it.
#' Returns NULL for every ordinary tabs run, so nothing about a report without
#' a categorical driver study changes.
#'
#' @param config_obj The tabs config object.
#' @return A single JSON string, or NULL.
#' @keywords internal
.read_catdriver_contribution <- function(config_obj) {
  path <- config_obj$catdriver_island
  if (is.null(path) || !nzchar(trimws(as.character(path)))) return(NULL)

  path <- as.character(path)
  if (!file.exists(path)) {
    cat(sprintf(paste0("\n[WARNING] catdriver_island points at a file that is not there: %s\n",
                       "  The report is built without the Categorical drivers tab.\n\n"), path))
    return(NULL)
  }
  ...
  ok <- tryCatch({
    parsed <- jsonlite::fromJSON(txt, simplifyVector = FALSE)
    identical(parsed$meta$kind, "catdriver")
  }, error = function(e) FALSE)

  if (!isTRUE(ok)) {
    cat(sprintf(paste0("\n[WARNING] %s is not a catdriver contribution file.\n",
                       "  The report is built without the Categorical drivers tab.\n\n"),
                basename(path)))
    return(NULL)
  }
...
#' a report without conjoint changes.
#'
#' @param config_obj The tabs config object.
#' @return A single JSON string, or NULL.
#' @keywords internal
.read_conjoint_contribution <- function(config_obj) {
  path <- config_obj$conjoint_island
  if (is.null(path) || !nzchar(trimws(as.character(path)))) return(NULL)

  path <- as.character(path)
  if (!file.exists(path)) {
    cat(sprintf(paste0("\n[WARNING] conjoint_island points at a file that is not there: %s\n",
                       "  The report is built without the Conjoint tab.\n\n"), path))
    return(NULL)
  }
  ...
  # Confirm it is what it claims to be before embedding it.
  ok <- tryCatch({
    parsed <- jsonlite::fromJSON(txt, simplifyVector = FALSE)
    identical(parsed$meta$kind, "conjoint")
  }, error = function(e) FALSE)

  if (!isTRUE(ok)) {
    cat(sprintf(paste0("\n[WARNING] %s is not a conjoint contribution file.\n",
                       "  The report is built without the Conjoint tab.\n\n"),
                basename(path)))
    return(NULL)
  }
...
if (.html_report_v2_on) {
  ...
  if (!is.null(data_layer_result) && data_layer_result$status == "PASS") {
    report_v2_result <- tryCatch({
      ...
      # A conjoint study for this project contributes a small JSON file when it
      # runs (write_conjoint_island, modules/conjoint/R/17_v2_island.R). Read it
