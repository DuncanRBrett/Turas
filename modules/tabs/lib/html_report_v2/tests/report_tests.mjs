...
run("the standard report-construction note replaces the old disclaimer field", () => {
  ...
  assert(h.indexOf("which includes R, Python and Javascript") >= 0,
    "the platform's languages are named");
  assert(h.indexOf("computed by the Turas software from the data") >= 0,
    "the deterministic claim renders");
  assert(h.indexOf("labelled and the model named") >= 0, "the AI-disclosure promise renders");
  assert(h.indexOf("reviewed by the report author") >= 0,
    "the author-review line renders");
...
run("the construction note describes BOTH engines, not just R", () => {
  ...
  // Every COMPUTED view is recalculated in the browser, so a reader told
  // "it runs in R" would be told something untrue.
  ...
  assert(h.indexOf("self-contained HTML document generated by Turas") >= 0,
...
run("a declaration replaces the WHOLE stock block, not just its first sentence", () => {
  ...
  ["computed by the Turas software from the data",
    "We use AI as a working tool",
    "labelled and the model named"].forEach((stock) => {
...
run("published-only earlier waves are called previously published", () => {
  ...
  assert(h.indexOf("Figures carried forward from earlier waves are shown as " +
    "previously published rather than recalculated") >= 0,
...
run("a wave that carries respondent data is not described as published-only", () => {
  ...
  assert(h.indexOf("shown as previously published") < 0,
...
run("a report with no earlier waves says nothing about waves at all", () => {
  ...
  assert(h.indexOf("held inside the file, using the same formulas.") >= 0,
    "and the recompute sentence closes cleanly without a wave clause");
});
...
run("AI is described as a working tool, and the author owns the report", () => {
  ...
  assert(h.indexOf("project management tools and report design") >= 0,
    "the selected-report uses are named");
  assert(h.indexOf("accountable for what") >= 0,
    "author accountability carries the assurance");
