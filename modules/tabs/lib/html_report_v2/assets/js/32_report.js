...
(function (global) {
   ...
   *
   * The default is Duncan's authored wording (set verbatim, 2026-08-13) and
   * describes a stock Turas report: R computes the PUBLISHED figures, and every
  ...
  function constructionHtml() {
    ...
    var produced = "This report was produced by " + company +
      ...
      "which includes R, Python and Javascript. ";
    ...
    return "<p>" + produced +
      "The published figures are computed in R.</p>" +
      "<p>The report itself is a self-contained HTML document generated by " +
      "Turas, with no third-party reporting engine involved. When you change a " +
      "view while reading, Turas recalculates it as you click: filters, custom " +
      "banners all recompute from the respondent-level data held inside the " +
      "file, using the same formulas.</p>" +
      ...
      (priorWavesArePublished()
        ? "<p>Figures carried forward from earlier waves are shown as " +
          "previously published rather than recalculated.</p>" : "") +
      "<p>We use AI as a working tool, as most analysts now do: in building " +
      "and maintaining Turas. In selected reports it may be used in project " +
      "management tools and report design. However, the report itself and the " +
      "data within is computed by the Turas software from the data. The report " +
      "has been reviewed by the report author, who is accountable for what " +
