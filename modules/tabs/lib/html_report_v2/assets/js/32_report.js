...
(function (global) {
  ...
  report.aboutHtml = function () {
    ...
    return '<div class="card"><h3>About this report</h3>' +
      ...
      "generated by Turas — no third-party reporting engine is involved. When " +
      "you change a view while reading — applying a filter, building a custom " +
      "banner, moving through the wave history — Turas recalculates it as you " +
      "click, from the respondent-level data held inside the file, using the " +
      "same formulas. Nothing is sent anywhere: the report works with no " +
      ...
      "<p>Turas is conventional statistical software, not an AI system. Every " +
      "figure Turas computes is deterministic and can be reproduced from the " +
      "source data. Figures carried forward from earlier waves are shown as " +
      "previously published rather than recalculated here, which is why the " +
      "report claims no significance against them. AI tools were used in " +
      "building and maintaining the Turas software, as they are in most modern " +
      "software development; no AI model takes part in any calculation.</p>" +
      "<p>Where AI is used to generate report content — insight callouts, " +
      "narrative summaries or comment themes — the report says so and names " +
      "the model. AI-drafted text works from the computed results; it does " +
      "not calculate anything.</p>" +
