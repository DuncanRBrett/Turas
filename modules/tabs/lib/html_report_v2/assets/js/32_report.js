...
(function (global) {
   ...
   * is unit-testable. */
  /**
   * How this report was built, in one paragraph.
   *
   * The default describes a stock Turas report and is accurate for one: R
   * computes the PUBLISHED figures, and every COMPUTED view and all wave-on-wave
   * significance are recalculated in the reader's browser by Turas's own engine.
   * Both are Turas, both are deterministic — but a reader told "it runs in R"
   * and nothing else would be told something untrue.
   *
  ...
  function constructionHtml() {
    ...
    return "<p>" + produced +
      ...
      "using the same formulas: filters and custom banners recompute from the " +
      "respondent-level data held inside the file" +
      // Waves were folded into the "recompute from respondent-level data" list,
      // which is not what happens: a wave loaded from published tables has no
      // respondent data in the file to recompute from, and the paragraph below
      // said so a few lines later. Named separately, and only when the report
      // actually carries earlier waves.
      (hasPriorWaves()
        ? ", and a wave comparison reads each earlier wave from the figures the " +
          "file carries for it" : "") +
      ". " +
      // "Nothing is sent anywhere" was true but sold the report short, because a
      // reader can reasonably wonder whether AI insights change it. They do not:
      // the report engine makes no network call of any kind — no fetch, no XHR,
      // no beacon — and any AI work happens in R when the file is BUILT.
      "Nothing is sent anywhere while you read it, and the report works with no " +
      "internet connection.</p>" +
      // Scoped to what Turas computes: waves loaded from published figures have
      // no source data in the file to reproduce them from.
      "<p>Every figure Turas computes for this wave is produced by code and can " +
      "be reproduced from the source data." +
      (priorWavesArePublished()
        ? " Figures carried forward from earlier waves are the numbers " +
          "published at the time, not recalculated from their original data." : "") +
      // The old text ended "…which is why no significance is claimed against
      // them", and that was simply untrue whenever tracking ran: the Tracking
      // tab reports significant wave-on-wave movements, the crosstab year-on-year
      // chips outline the significant ones, and the auto-methodology block below
      // describes the test. The About card contradicted itself on the same page.
      (hasPriorWaves()
        ? " Where this wave is compared with an earlier one, the comparison is " +
          "tested on the figures and bases each wave carries." : "") +
      "</p>" +
