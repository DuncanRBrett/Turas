...
(function (global) {
  ...
  report.aboutHtml = function () {
    ...
    return '<div class="card"><h3>About this report</h3>' +
      ...
      // "Statistical analysis runs in R" was only half the story and read as the
      // whole of it: R computes the PUBLISHED figures, but every COMPUTED view
      // and all wave-on-wave significance are recalculated in the reader's
      // browser by Turas's own engine. Both are Turas, both are deterministic —
      // but a reader told "it runs in R" would be told something untrue.
      "<p>This report was produced by " + company + " using Turas Analytics, " +
      "our in-house analysis and reporting platform. The published figures are " +
      ...
      "<p>Every number here is produced by code and can be reproduced from the " +
      "source data. Figures carried forward from earlier waves are shown as " +
      "previously published rather than recalculated, which is why no " +
      "significance is claimed against them.</p>" +
      // Honest about AI as a WORKING TOOL, not only as a product feature. The
      // old note promised "the report says so and names the model" for any AI
      // report content, but that promise is wired to TR.ai.methodologyHtml(),
      // which fires only when Turas's own insights run. AI-drafted comment
      // marks and config-authored sections reach the report by other routes and
      // would never have triggered it. Author accountability is also a more
      // durable assurance than an absolute no-AI claim, which ages badly and
      // has to be rewritten every time a workflow changes.
      "<p>We use AI as a working tool, as most analysts now do: in building and " +
      "maintaining Turas, and in first-pass drafting and sifting. It calculates " +
      "nothing. Every figure in this report is computed by the software from " +
      "the data. And it decides nothing. Every selection, interpretation and " +
      "word has been reviewed by the report author, who is accountable for what " +
  ...
  /** The report's confidence level as a whole number, from the CONFIGURED alpha
   *  rather than a hard-coded 95 — a project on alpha = 0.10 letters its tables
   *  at 90%, and a methodology note that still said 95% would describe letters
   *  the report does not show. `secondary` gives the dual-significance level. */
  function sigPct(secondary) {
    var a = secondary ? TR.stats.alphaSecondary() : TR.stats.alphaPrimary();
    return String(Math.round((1 - a) * 100));
  }
  ...
  function autoAboutHtml() {
    ...
    return "<h3>Methodology (auto-generated)</h3>" +
      ...
      "COMPUTED. Significance: columns are compared at the " + sigPct() +
      "% level (optionally also " + sigPct(true) + "%, shown as lowercase " +
      "letters)" +
      // Bonferroni is on unless the project explicitly disables it, and it
      // materially raises the bar — omitting it understated what a letter means.
      (TR.stats.bonferroni()
        ? ", with a Bonferroni correction across every pairwise comparison " +
          "within a banner group" : "") +
      ". Percentages use a two-proportion pooled z-test, which needs an " +
      "expected count of at least 5 both in and out of the category; means, " +
      "indexes and NPS use a Welch t-test. Bases under " + p.low_base_threshold +
      " are excluded from testing and flagged ⚠. " +
      // Wave tests are single planned comparisons, so they deliberately do NOT
      // take the Bonferroni divisor (22w_waves.js calls zPrimary(1)). Saying so
      // stops the Tracking tab and the crosstab columns looking inconsistent.
      (TR.PREV && TR.PREV.waves && TR.PREV.waves.length > 1
        ? "Wave-on-wave change is tested at the same level without the " +
          "Bonferroni correction, each being a single planned comparison " +
          "rather than one of a family. " : "") +
