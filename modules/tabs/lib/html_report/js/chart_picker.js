...
/**
 * Copy chart as PNG to clipboard (for pasting into PowerPoint).
 * Same rendering as exportChartPNG but writes to clipboard.
 */
function copyChartToClipboard(qCode) {
  var container = document.querySelector(".question-container.active");
  if (!container) return;
  var wrapper = container.querySelector(".chart-wrapper");
  if (!wrapper) return;
  ...
  var gWrap = document.createElementNS("http://www.w3.org/2000/svg", "g");
  gWrap.setAttribute("transform", "translate(0," + titleBlockH + ")");
  while (svgClone.firstChild) gWrap.appendChild(svgClone.firstChild);
  svgClone.appendChild(gWrap);

  var bgRect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
  bgRect.setAttribute("x", "0"); bgRect.setAttribute("y", "0");
  bgRect.setAttribute("width", origW); bgRect.setAttribute("height", newH);
  bgRect.setAttribute("fill", "#ffffff");
  svgClone.insertBefore(bgRect, svgClone.firstChild);
  ...
  var copyBtn = container.querySelector('button[title*="Copy chart"]');

  var img = new Image();
  img.onerror = function() {
    URL.revokeObjectURL(url);
    alert("Clipboard copy failed. Try the Export Chart button instead.");
  };
  img.onload = function() {
    var canvas = document.createElement("canvas");
