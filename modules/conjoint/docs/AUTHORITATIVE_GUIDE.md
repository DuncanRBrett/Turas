...
**Version:** 3.0.0 **Last Updated:** March 2026 **Audience:**
Statisticians, Senior Analysts, Market Researchers
...
The Turas Conjoint Module estimates consumer preferences through
choice-based experiments. It implements the random utility maximization
(RUM) framework where consumers are assumed to choose the alternative
...
## Conjoint Analysis Fundamentals {#conjoint-analysis-fundamentals}
...
Where: - `U_ij` = Total utility of alternative j for person i - `V_ij` =
Systematic (observable) utility component - `ε_ij` = Random
...
Where: - `β_k` = Part-worth utility for attribute level k - `X_ijk` = 1
if alternative j has level k, 0 otherwise
...
**Key Properties:** - Maximum likelihood estimation - Consistent and
asymptotically efficient - Requires unique (chid, alt) identifiers -
...
**Key Properties:** - Based on Cox proportional hazards framework - More
robust convergence - Faster computation - Fewer diagnostics than mlogit
...
## Part-Worth Utilities {#part-worth-utilities}
...
**Why Effects Coding:** - Utilities sum to zero within attribute - No
arbitrary baseline level - More intuitive interpretation - Standard in
conjoint analysis
...
**Properties:** - Sum of utilities per attribute = 0 - Differences
between levels preserved - Enables importance calculation
...
**Interpretation:** - Percentage of total utility "swing" from each
attribute - Higher = more influence on choice - Sum across all
attributes = 100%
...
Given product configurations, market share is calculated using logit
probabilities:
...
Where: - `U_j` = Total utility of product j (sum of part-worths for its
features) - Sum is over all products in the simulation
...
Where: - `LL_model` = Log-likelihood of fitted model - `LL_null` =
Log-likelihood of null model (equal probabilities)
...
**Note:** McFadden R² is typically lower than OLS R². Values of 0.2-0.4
...
**Calculation:** 1. For each choice set, predict the alternative with
highest probability 2. Compare to actual choice 3. Hit rate = proportion
of matches
...
## Alchemer Integration {#alchemer-integration}

### Overview

Version 2.1 adds direct import of Alchemer CBC exports, eliminating
...
1.  **Realistic Trade-Offs:** Forces respondents to make realistic
    choices
...
1.  **IIA Assumption:** Independence of Irrelevant Alternatives
    -   Adding/removing alternatives shouldn't change relative
        preferences
...
Core calculations use base R: - `optim()` - Optimization - `vcov()` -
Variance-covariance extraction - `qnorm()` - Normal quantiles for CIs
...
**Random Utility Theory:** - McFadden, D. (1974). Conditional logit
analysis of qualitative choice behavior. In P. Zarembka (Ed.),
*Frontiers in Econometrics*. Academic Press.

**Conjoint Analysis:** - Green, P. E., & Srinivasan, V. (1978). Conjoint
analysis in consumer research: Issues and outlook. *Journal of Consumer
Research*, 5(2), 103-123.

**Choice Modeling:** - Train, K. E. (2009). *Discrete Choice Methods
with Simulation* (2nd ed.). Cambridge University Press.

### R Package Documentation

**mlogit:** - Croissant, Y. (2020). Estimation of Random Utility Models
in R: The mlogit Package. *Journal of Statistical Software*, 95(11).

**survival:** - Therneau, T. M. (2023). A Package for Survival Analysis
in R. <https://CRAN.R-project.org/package=survival>

### Market Research Standards

**ESOMAR:** - ESOMAR Guidelines on Conjoint Analysis and Pricing
Research.
...
*Turas Conjoint Module v2.1.0* *Last Updated: December 2025*
