...
| File | Change |
|---|---|
| `modules/brand/lib/html_report/03_page_builder.R` | Add `.BR_PRIMARY_SUBTABS` constant; add pure `build_br_subtab_nav(flat_tabs, cat_id)`; replace the inline nav block in `build_br_category_panel()` with a call to it; add `.br-subtab-sep` / `.br-subtab-grouplabel` / `.br-subtab-btn--appendix` CSS in `build_brand_page()`. |
...
- **Duncan eyeballs (after `launch_turas()` regen):** (1) each category opens on **Brand Funnel** as before; (2) the primary tabs (Funnel · MA Metrics · Category Buying · Word of Mouth) sit together, left of a faint **"Detail"** divider; (3) the appendix tabs sit right of the divider, slightly muted; (4) clicking any tab — primary or appendix — still switches correctly. If the divider/spacing looks off, that's a CSS tweak — tell me and I'll adjust (I can't see it).
