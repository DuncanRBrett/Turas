...
**Required positions** (relative, so slot into whatever admin/intro pages you already have):
...
Only ONE hidden value is needed: `hv_focal_cat`. Its numeric question ID goes into the JS at `DEST_QID` (currently set to 12 in test survey 8839203, but verify in Build view).
