# ==============================================================================
# 1BRAND SYNTHETIC EXAMPLE - Brand_Config.xlsx GENERATOR
# ==============================================================================
# Generates a fully-filled Brand_Config.xlsx for the Ina Paarman's Kitchen
# Dry Seasonings & Spices single-category synthetic study. Uses the shared
# visual-polish template infrastructure (write_settings_sheet /
# write_table_sheet from modules/shared/template_styles.R).
#
...
# ==============================================================================
# SETTINGS DEFINITION (filled defaults, IPK Dry Seasonings & Spices)
# ==============================================================================

#' Build the IPK-specific settings definition
#'
#' Returns the same structure as .build_brand_settings_def() in
#' generate_config_templates.R but with all `default` values filled in for
#' the IPK Dry Seasonings & Spices study.
#'
#' @return List of section definitions
#' @keywords internal
.build_1brand_settings_def <- function() {
  ...
  list(
    ...
    list(
      section_name = "MULTI-CATEGORY ROUTING (single-category study: accept defaults)",
      fields = list(
        list(name = "focal_assignment", required = TRUE, default = "balanced",
             description = "[REQUIRED] How respondents are assigned to their focal category. Single-category studies: 'balanced'",
        ...
        list(name = "focal_category_col", required = FALSE, default = "Focal_Category",
             description = "[Optional] Column in data file containing pre-assigned focal category",
             valid_values_text = "Column name in data file"),
        list(name = "cross_category_awareness", required = FALSE, default = "N",
             description = "[Optional] Collect brand awareness for all qualified categories. Single-category: N",
             valid_values_text = "Y or N", dropdown = c("Y", "N")),
        list(name = "cross_category_pen_light", required = FALSE, default = "N",
             description = "[Optional] Collect light brand penetration for non-focal categories. Single-category: N",
             valid_values_text = "Y or N", dropdown = c("Y", "N"))
      )
    ),
    ...
    list(
      ...
      fields = list(
        list(name = "element_funnel", required = FALSE, default = "Y",
             description = "[Optional] Brand funnel derived from awareness + attitude + penetration",
             valid_values_text = "Y or N", dropdown = c("Y", "N")),
        ...
        list(name = "element_dba", required = FALSE, default = "Y",
             description = "[Optional] Distinctive Brand Assets: Fame x Uniqueness grid (adds +2 min)",
             valid_values_text = "Y or N", dropdown = c("Y", "N")),
        list(name = "element_portfolio", required = FALSE, default = "N",
             description = "[Optional] Portfolio analysis. Requires 2+ categories; single-category study: N",
             valid_values_text = "Y or N", dropdown = c("Y", "N")),
        list(name = "element_wom", required = FALSE, default = "Y",
             description = "[Optional] Word-of-Mouth: received/shared x positive/negative (adds +2 min)",
             valid_values_text = "Y or N", dropdown = c("Y", "N"))
      )
    ),
    ...
    list(
      ...
      fields = list(
        list(name = "wom_timeframe", required = FALSE, default = cat_def$timeframe_target,
    ...
    list(
      ...
      fields = list(
        ...
        list(name = "alpha_secondary", required = FALSE, default = "",
             ...
             valid_values_text = "0.01 to 0.20 or blank", numeric_range = c(0.01, 0.20)),
        ...
        list(name = "low_base_warning", required = FALSE, default = 75,
             description = "[Optional] Base size below which a low-base warning is shown",
    ...
    list(
      ...
      fields = list(
        list(name = "colour_focal", required = FALSE, default = "#C8102E",
             description = "[Optional] Saturated colour for focal brand in charts. IPK brand red",
        ...
        list(name = "colour_competitor", required = FALSE, default = "#B0B0B0",
             description = "[Optional] Desaturated colour for competitor brands (design principle 3)",
             valid_values_text = "Hex colour"),
    ...
    list(
      ...
      fields = list(
        ...
        list(name = "report_subtitle", required = FALSE,
             default = sprintf("Wave %d Baseline - %s", meta$wave, cat_def$name),
             description = "[Optional] Subtitle for the report header",
             valid_values_text = "Free text"),
        list(name = "show_about_section", required = FALSE, default = "Y",
             description = "[Optional] Include About & Methodology section",
             valid_values_text = "Y or N", dropdown = c("Y", "N")),
        list(name = "structure_file", required = TRUE, default = "Survey_Structure.xlsx",
             description = "[REQUIRED] Path to Survey_Structure.xlsx (relative to this config)",
...
# ==============================================================================
# CATEGORIES SHEET (one row: Dry Seasonings & Spices)
# ==============================================================================

.build_1brand_categories_rows <- function() {
  cat_def <- ipk_category()
  list(
    list(
...
.build_1brand_dba_assets_rows <- function() {
  lapply(ipk_dba_assets(), function(a) {
    list(
      AssetCode  = a$code,
      AssetLabel = a$label,
...
#' Generate the filled Brand_Config.xlsx for the IPK 1Brand example
#'
...
#' @export
generate_1brand_config <- function(output_path, overwrite = TRUE) {
  ...
  write_settings_sheet(
    wb, "Settings",
    .build_1brand_settings_def(),
    title = "TURAS Brand Module - Configuration",
    subtitle = "Ina Paarman's Kitchen - Dry Seasonings & Spices (synthetic single-category example)"
  )
  ...
  write_table_sheet(
    ...
    subtitle = "Single-category study: exactly one row.",
    example_rows = .build_1brand_categories_rows(),
    num_blank_rows = 0
  )
  ...
  write_table_sheet(
    ...
    subtitle = "Five distinctive brand assets for Ina Paarman's Kitchen.",
    example_rows = .build_1brand_dba_assets_rows(),
    num_blank_rows = 0
  )
