...
# (these are defined in 1brand/03_structure.R; replicated here to keep
#  the 3cat example self-contained)
# ==============================================================================
...
.build_3cat_project_settings_def <- function() {
...
.build_3cat_questions_rows <- function() {
  ...
  .slot_cols  <- function(cat_code) length(cat3_brands(cat_code)) + 1L
  .brand_cols <- function(cat_code) length(cat3_brands(cat_code))

  # --- Category-level questions (one set per category) ---
  cat_qs <- unlist(lapply(cat3_categories(), function(cat) {
  ...
  # --- CEP questions (15 per category, globally unique codes) ---
  cep_qs <- unlist(lapply(cat3_categories(), function(cat) {
    lapply(cat3_ceps(cat$code), function(cep) list(
  ...
  # --- Attribute questions (5 per category, category-prefixed codes) ---
  # e.g. DSS_ATTR01 = "Good value for money" in DSS context
  attr_qs <- unlist(lapply(cat3_categories(), function(cat) {
    lapply(cat3_attributes(), function(attr) list(
  ...
  # Brand codes are globally unique so WOM_{BRAND} columns don't conflict.
  # Each respondent answers WOM only for brands in their focal category.
  # WOM is asked across every category's brands at once, so its slot count is
  # the total distinct brand list plus one for NONE.
  wom_brands <- length(unique(unlist(lapply(cat3_categories(), function(cat)
    vapply(cat3_brands(cat$code), function(b) b$code, character(1))))))
  ...
  # --- DBA battery (brand-level; shown to all respondents, IPK assets only) ---
  dba_qs <- unlist(lapply(cat3_dba_assets(), function(a) list(
...
.build_3cat_brands_rows <- function() {
  unlist(lapply(cat3_categories(), function(cat) {
    lapply(cat3_brands(cat$code), function(b) list(
      Category     = cat$name,
      ...
      Colour       = cat3_brand_colour(b$code)
    ))
  }), recursive = FALSE)
}
...
# ==============================================================================
# CEPs SHEET  (45 rows: 15 per category)
# ==============================================================================

.build_3cat_ceps_rows <- function() {
  unlist(lapply(cat3_categories(), function(cat) {
    mapply(function(cep, i) list(
      ...
      CEPText      = cep$text,
      DisplayOrder = i
    ), cat3_ceps(cat$code), seq_along(cat3_ceps(cat$code)), SIMPLIFY = FALSE)
  }), recursive = FALSE)
}


# ==============================================================================
# ATTRIBUTES SHEET  (15 rows: 5 per category, same text but category-specific codes)
# ==============================================================================

.build_3cat_attrs_rows <- function() {
  unlist(lapply(cat3_categories(), function(cat) {
    mapply(function(attr, i) list(
      Category     = cat$name,
      CategoryCode = cat$code,
      AttrCode     = sprintf("%s_%s", cat$code, attr$code),   # e.g. DSS_ATTR01
      AttrText     = attr$text,
      DisplayOrder = i
    ), cat3_attributes(), seq_along(cat3_attributes()), SIMPLIFY = FALSE)
  }), recursive = FALSE)
}
...
.build_3cat_dba_structure_rows <- function() {
...
# ==============================================================================
# QUESTIONMAP SHEET  (role-registry: maps semantic roles to client question codes)
# ==============================================================================

.build_3cat_questionmap_rows <- function() {

  # System rows (once, not per category)
  system_rows <- list(
    ...
    list(Role = "system.focal.category",
         ...
         ColumnPattern = "{code}", OptionMapScale = "", Notes = "DSS, PAS, or SLD")
  )

  # Funnel rows per category
  funnel_rows <- unlist(lapply(cat3_categories(), function(cat) {
  ...
  # WOM rows (brand-level, shared across categories)
  wom_rows <- list(
  ...
  c(system_rows, funnel_rows, wom_rows, dba_rows)
}
...
.build_3cat_optionmap_rows <- function() {
  list(
    # Attitude scale (Romaniuk 5-position)
...
#' Generate the filled Survey_Structure.xlsx for the IPK 3-category example
#'
...
#' @export
generate_3cat_structure <- function(output_path, overwrite = TRUE) {
  ...
  write_settings_sheet(
    wb, "Project",
    .build_3cat_project_settings_def(),
  ...
  write_table_sheet(
    ...
    subtitle = "All CBM questions across 3 categories (DSS, PAS, SLD) plus brand-level WOM and DBA batteries.",
    example_rows   = .build_3cat_questions_rows(),
    num_blank_rows = 0
  )
  ...
  write_table_sheet(
    ...
    subtitle = "Labels for coded responses. Attitude and category-buying scales replicated per category; WOM count and DBA fame scales are shared.",
    example_rows   = .build_3cat_options_rows(),
    num_blank_rows = 0
  )
  ...
  write_table_sheet(
    ...
    subtitle = "30 brand entries: 10 brands x 3 categories. IPK is focal in each category. Some brands appear in multiple categories (e.g. Knorr in DSS + PAS).",
    example_rows   = .build_3cat_brands_rows(),
    num_blank_rows = 0
  )
  ...
  write_table_sheet(
    ...
    subtitle = "45 CEPs total: 15 per category. CEP codes are globally unique (DSS=CEP01-15, PAS=CEP16-30, SLD=CEP31-45).",
    example_rows   = .build_3cat_ceps_rows(),
    num_blank_rows = 0
  )
  ...
  write_table_sheet(
    ...
    subtitle = "5 attributes per category (15 rows total). Same 5 perception items across all categories; codes are category-prefixed (e.g. DSS_ATTR01) to keep data columns distinct.",
    example_rows   = .build_3cat_attrs_rows(),
    num_blank_rows = 0
  )
  ...
  write_table_sheet(
    ...
    subtitle = "5 distinctive brand assets for Ina Paarman's Kitchen. DBA is brand-level: all respondents across all categories see these assets.",
    example_rows   = .build_3cat_dba_structure_rows(),
    num_blank_rows = 0
  )
  ...
  write_table_sheet(
    ...
    subtitle = "Maps semantic roles to client question codes. Funnel roles are category-qualified (e.g. funnel.awareness.DSS). See ROLE_REGISTRY.md.",
    example_rows   = .build_3cat_questionmap_rows(),
    num_blank_rows = 0
  )
  ...
  write_table_sheet(
    ...
    subtitle = "Maps coded values to semantic position roles. attitude_scale is shared across all categories.",
    example_rows   = .build_3cat_optionmap_rows(),
    num_blank_rows = 0
  )
