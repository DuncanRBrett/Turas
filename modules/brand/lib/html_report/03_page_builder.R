...
#' When the study was in field, as the analyst typed it
#'
#' Free text from the \code{fieldwork_dates} Settings key, for example
#' "19 to 26 May 2026". Nothing parses it: a fieldwork period is written a
#' dozen ways and a parser would refuse half of them, while the only job
#' here is to put the study's own date on the page next to the wave.
#'
#' @return The trimmed string, or "" when the key is blank or absent.
#' @keywords internal
.br_fieldwork_dates <- function(config) {
  v <- config$fieldwork_dates
...
#'
#' The badge bar used to end with a bare month and year, which was
#' \code{Sys.Date()}: the day the report was written, not the day the study
#' was in field. Beside "Wave 1" it read as the wave's date, and a client
#' whose fieldwork ran in May was looking at a report that appeared to say
#' September. Both dates now carry a label, and the study's own date is the
#' one that comes first.
#'
#' A study that has not filled in \code{fieldwork_dates} shows the generation
#' date alone, still labelled. No date is invented for it.
#' @keywords internal
build_br_header <- function(config) {
  brand <- config$colour_focal %||% "#1A5276"
  sep <- '<span style="opacity:0.4;">|</span>'
  badges <- c(
    sprintf('<span>Client: %s</span>', .br_esc(config$client_name %||% "")),
    sprintf('<span>Focal Brand: %s</span>', .br_esc(config$focal_brand %||% "")),
    sprintf('<span>Wave %s</span>', .br_esc(config$wave %||% 1)))
  fw <- .br_fieldwork_dates(config)
  if (nzchar(fw)) {
    badges <- c(badges, sprintf(
      '<span data-br-fieldwork>Fieldwork: %s</span>', .br_esc(fw)))
  }
  badges <- c(badges, sprintf(
    '<span data-br-generated>Generated %s</span>',
    format(Sys.Date(), "%B %Y")))
  badge_bar <- paste(badges, collapse = paste0("\n    ", sep, "\n    "))
  sprintf('
...
build_br_about_panel <- function(config) {
  ...
  # The study's own date, said in the same words as the header badge, so the
  # two cannot be read as different claims. Absent when nobody filled the key
  # in: an unfilled date is left unfilled rather than guessed from the clock.
  fw <- .br_fieldwork_dates(config)
...
<div class="br-panel" id="panel-about">
  <div class="br-section" style="max-width:800px;">
    ...
    <div style="font-size:13px;line-height:1.7;color:#475569;">
      ...
      %s
      <p style="margin-top:20px;font-size:11px;color:#94a3b8;">Report generated by TURAS Analytics Platform v%s on %s. That is when this file was written, not when the study was in field.</p>
    </div>
  </div>
</div>',
    background_html,
