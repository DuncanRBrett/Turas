# Turas Hub App: Developer Implementation Considerations

**Target Environment:**  Local R/Plumber Server + Vanilla JS Frontend  **Focus:**  Solving Cross-Origin, Persistence, and High-Fidelity Rendering

----------

## 1. Origin & Security (The Iframe Bridge)

The most critical technical hurdle is ensuring the Hub App (parent) can talk to the Turas Reports (iframes).

-   **Unified Origin:**  The Plumber server must serve the Hub App assets (HTML/JS) and the Report HTML files from the same port and protocol. If they differ, the browser will block the  `postMessage`  or DOM access required for pinning.
    
-   **Path Validation:**  To prevent directory traversal attacks, the R backend must strictly validate that any  `path`parameter requested via  `/api/report`  resides within the user-defined  `project_dirs`.
    
-   **Header Injection:**  Since reports are designed as standalone files, the developer must use a "Bridge Script" injected into the iframe to hide the original report navigation/header so it doesn't clash with the Hub App UI.
    

## 2. Pinning & Visual Fidelity

-   **The Snapshot Dilemma:**  Pinned tables often lose styling because they rely on external CSS classes.
    
-   **Shadow Rendering:**  For table pins, the developer should implement a "headless" render:
    
    1.  Load the source report's CSS into a hidden container.
        
    2.  Inject the table HTML.
        
    3.  Use a library like  `html2canvas`  (or a native browser  `captureVisibleTab`  if moving to Electron) to create a high-DPI PNG for the PowerPoint export.
        
-   **MutationObserver:**  The app should "listen" to the report's internal state. When a user clicks "Pin" inside an iframe, the parent Hub should detect that change and pull the data immediately into IndexedDB.
    

## 3. Data Integrity & Syncing

-   **IndexedDB as Cache:**  Use IndexedDB for "snappy" UI interactions (loading pins instantly on app launch).
    
-   **JSON as Source of Truth:**  Every change to a pin or annotation must be mirrored to the  `.turas_pins.json`  sidecar file. This ensures that if a user clears their browser cache, the data remains safe in the project folder.
    
-   **Race Conditions:**  If the user has multiple tabs open, the developer must implement a "Debounced Write" (e.g., wait 500ms after the last keystroke) to avoid corrupting the JSON sidecar files during heavy editing.
    

## 4. Performance & Memory Management

-   **Iframe Eviction (LRU):**  Loading 15 reports (each 5-10MB) will crash the browser's memory. The developer should implement a "Least Recently Used" (LRU) cache—only keep the 5 most recently viewed reports active in the DOM; swap the rest to "placeholder" states.
    
-   **Lazy Loading:**  Do not load the iframe until the user actually clicks the tab for that specific report.
    

## 5. R/Plumber Backend Logic

-   **Dependency Check:**  The launch script (`launch_hub_app.R`) must verify that  `plumber`  and  `officer`  are installed. If missing, it should prompt the user to install them to ensure "non-developer" users like Jess can run it.
    
-   **Port Scanning:**  If the default port (e.g., 7860) is taken, the R script should automatically increment and find the next available port to prevent startup failures.
    

## 6. Export Engine (Officer)

-   **PPTX Templating:**  The  `officer`  package works best when it has a "Reference PPTX". The developer needs to create a  `template.pptx`  that includes the Research LampPost branding, which the code will then "fill" with content.
    
-   **Native Tables vs. Images:**  For the PowerPoint export, the developer should attempt to convert  `tableHtml`  into a native PPTX table object first for editability, falling back to the  `tablePng`  only if the layout is too complex.
